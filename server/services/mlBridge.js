...
// Path to the Python disease prediction script
...
/**
 * Call the Python ML disease prediction script via child_process.spawn
 *
 ...
 * @returns {Promise<Object>} Disease prediction result JSON
 */
const predictCrop = async (imagePath) => {
  ...
  return new Promise((resolve, reject) => {
    ...
    // Spawn Python process asynchronously without blocking the Event Loop
    const python = spawn(pythonCmd, args, {
      timeout: 45000, // 45 second timeout (deep learning models can take longer to load)
    });
    ...
    // Collect error chunks from stderr
    ...
    python.on('close', (code) => {
      ...
      console.log(stderr);
      if (code !== 0) {
        console.error(`Python disease prediction process exited with code ${code}`);
      ...
      try {
        ...
        console.error('Failed to parse ML disease output:', stdout);
        resolve({
          error: 'Failed to parse ML model output as JSON'
        });
      }
    });
    ...
    python.on('error', (error) => {
      console.error('Failed to spawn Python process for disease prediction:', error);
      resolve({
        error: `Failed to start ML model: ${error.message}. Ensure Python and TensorFlow are installed.`
      });
    });
  });
};
